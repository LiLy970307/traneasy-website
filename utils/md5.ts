...
const convertToWordArray = (value: string) => {
  ...
  const words = new Array<number>(wordCount - 1).fill(0);
  let bytePosition = 0;

  for (let index = 0; index < length; index += 1) {
    const wordIndex = (index - (index % 4)) / 4;
    bytePosition = (index % 4) * 8;
    words[wordIndex] = words[wordIndex] | (value.charCodeAt(index) << bytePosition);
  }
...
export const md5 = (value: string) => {
  const words = convertToWordArray(utf8Encode(value));
  let a = 0x67452301;
  let b = 0xefcdab89;
  let c = 0x98badcfe;
  let d = 0x10325476;
